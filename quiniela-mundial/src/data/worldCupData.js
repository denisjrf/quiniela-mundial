...
export function getMatchKickoff(matchId) {
  ...
  if (!isNaN(numericId) && numericId >= 1 && numericId <= 72) {
    const dayIndex = Math.floor((numericId - 1) / 6); // 6 partidos al día
    const matchIndexInDay = (numericId - 1) % 6;
    
    // Horarios distribuidos: 13:00, 15:00, 17:00, 19:00, 21:00, 23:00 UTC
    const hoursUTC = 13 + matchIndexInDay * 2;
...
// Inyectar dinámicamente kickoff
