...
router.get('/', auth, async (req, res) => {
  try {
    // 1. Obtener todas las predicciones y nombres de usuarios
    const allPredictions = await db.query(`
      SELECT p.user_id, u.name, p.group_predictions, p.knockout_predictions 
      FROM predictions p 
      ...
      rounds.forEach(round => {
        knockoutPred[round]?.forEach(pred => {
          ...
          if (pred.winner === real.winner) {
            points += 3; // 3 puntos por cada ganador de eliminatorias acertado
            exactHits++;
          }
        });
      });
